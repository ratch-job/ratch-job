    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(178) => msg.task_code = r.read_string(bytes).map(Cow::Borrowed)?,
...
impl<'a> MessageWrite for JobDo<'a> {
    fn get_size(&self) -> usize {
        ...
        + if self.task_code == "" { 0 } else { 2 + sizeof_len((&self.task_code).len()) }
    }
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.task_code != "" { w.write_with_tag(178, |w| w.write_string(&**&self.task_code))?; }
        Ok(())
    }
}
