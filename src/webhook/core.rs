...
impl WebHookManager {
    ...
    pub fn update_event_map(&mut self, app: AppKey, event: NotifyEvent, object: WebHookObject) -> anyhow::Result<()> {
            ...
            if let Some(object) = map.get(&event) {
                match event {
                    NotifyEvent::ExeJobFail(fail_msg) => {
                        match &object.hook_source {
                            WebHookSource::WeiXin => {
                                let wx = WeChatChannel { webhook_url: object.url.clone()};
